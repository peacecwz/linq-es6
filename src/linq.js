(function (global) {
    ...
    var last = function (nullable, selector) {
        return function (fn) {
            fn = fn || selector;
            for (let item of this.toArray().reverse()) {
                if (fn(item)) {
                    return item;
                }
            }
    ...
    generator.last = last(false, fnTrue);
    generator.lastOrDefault = last(true, fnTrue);
